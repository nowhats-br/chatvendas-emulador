...
export interface OrderPayload {
  ...
  customerId?: string;
  customerName?: string;
  ticketId?: string; // Vínculo com o Ticket
  createdAt: string;
}
...
class OrderServiceClass {
  ...
  getByTicketId(ticketId: string): Order[] {
    return this.orders.filter(o => o.ticketId === ticketId);
  }
  ...
  async createOrder(payload: OrderPayload): Promise<{ success: boolean; orderId: string }> {
    console.log(`[OrderService] Processando nova venda para Ticket: ${payload.ticketId || 'N/A'}...`);
