    ...
    try {
      ...
      try {
        socket = makeWASocket({
          ...
          browser: Browsers?.macOS ? Browsers.macOS('ChatVendas-Whaileys') : ['ChatVendas', 'macOS', '1.0.0'],
    ...
    for (const message of messages) {
      try {
        ...
        if (!ticket) {
          ...
          console.log(`✅ WhaileysManager: Novo ticket criado: ${ticketId} (Contato: ${contact.name})`);
          ...
          // Se encontrou ticket, mas está associado a outro ID de contato
          if (ticket.contact_id !== contact.id) {
            console.log(`⚠️ WhaileysManager: Ticket encontrado (${ticket.id}) pertence a outro contact_id duplicado. Vinculando mensagens a ele.`);
          }
          console.log(`ℹ️ WhaileysManager: Usando ticket existente ${ticket.id} (${ticket.status})`);
        }
