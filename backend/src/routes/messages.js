...
router.post('/send-quick', upload.single('media'), async (req, res) => {
  try {
    let { phoneNumber, messageType, content } = req.body;

    // Se vier via FormData (com a flag de media ou não), content pode ser string JSON
    if (typeof content === 'string') {
      try {
        content = JSON.parse(content);
      } catch (e) {
        // Se não for JSON, assumimos que é texto para mensagem simples, 
        // mas se for text type, normalizamos.
        if (messageType === 'text') {
    ...
    if (!phoneNumber || !messageType) {
      return res.status(400).json({
        error: 'phoneNumber e messageType são obrigatórios'
      });
    }

    const db = getDatabase();

    // Buscar uma instância conectada (priorizar Whaileys para interativos)
    let instance;
    if (['buttons', 'list', 'carousel'].includes(messageType)) {
    ...
    if (!contact) {
      ...
      await db.run(`INSERT INTO contacts (id, name, phone, whatsapp_id) VALUES (?, ?, ?, ?)`,
    ...
    let ticket = await db.get(`SELECT * FROM tickets WHERE contact_id = ? AND instance_id = ? AND status != 'closed' ORDER BY created_at DESC LIMIT 1`, [contact.id, instance.id]);
    if (!ticket) {
      const ticketId = uuidv4();
      await db.run(`INSERT INTO tickets (id, contact_id, instance_id, status, subject) VALUES (?, ?, ?, 'open', 'Mensagem Rápida')`, [ticketId, contact.id, instance.id]);
    ...
    let mediaUrl = null, mediaFilename = null;
    let waMessage = {};
    let messageContent = '';
    ...
    switch (messageType) {
      case 'text':
        waMessage = { text: content.text || content };
      ...
      case 'buttons':
        if (instance.provider === 'whaileys') {
          ...
          waMessage = { text: content.text, footer: content.footer || '', buttons: whaileysButtons };
        } else {
      ...
      case 'list':
        if (instance.provider === 'whaileys') {
          ...
          waMessage = {
            text: content.text, footer: content.footer || '', title: content.title || '',
            buttonText: content.buttonText || 'Abrir Menu', sections: whaileysSections
          };
        } else {
          waMessage = { text: JSON.stringify(content.sections) }; // Fallback simplificado
        }
        messageContent = JSON.stringify(content);
        break;

      case 'carousel':
        // Carrossel agora é suportado
        // content pode ser array de cards
        const cards = Array.isArray(content) ? content : (content.cards || content.carousel || []);
        waMessage = { carousel: cards };
        if (instance.provider !== 'whaileys') {
          return res.status(400).json({ error: 'Carrossel requer instância whaileys' });
        }
        messageContent = JSON.stringify(cards);
        break;

      case 'image':
        if (req.file) {
          const imageBuffer = await fs.readFile(req.file.path);
          waMessage = { image: imageBuffer, caption: content.caption || '' };
        } else if (content.url || typeof content === 'string') {
          const url = content.url || content;
          waMessage = { image: { url }, caption: content.caption || '' };
          mediaUrl = url;
          mediaFilename = 'image.jpg';
        } else {
          return res.status(400).json({ error: 'Arquivo ou URL obrigatório para imagem' });
        }
        messageContent = content.caption || 'Imagem';
        break;

      case 'video':
        if (req.file) {
          waMessage = { video: { url: path.resolve(req.file.path) }, caption: content.caption || '' };
        } else if (content.url || typeof content === 'string') {
          const url = content.url || content;
          waMessage = { video: { url }, caption: content.caption || '' };
          mediaUrl = url;
        } else {
          return res.status(400).json({ error: 'Arquivo ou URL obrigatório para vídeo' });
        }
        messageContent = content.caption || 'Vídeo';
        break;

      case 'audio':
        if (req.file) {
          waMessage = {
            audio: { url: path.resolve(req.file.path) },
          ...
          return res.status(400).json({ error: 'Arquivo ou URL obrigatório para áudio' });
        }
        messageContent = 'Áudio';
        break;

      default: return res.status(400).json({ error: 'Tipo não suportado' });
    }

    // Enviar mensagem via WhatsApp
    let sentMessage;
    try {
      if (messageType === 'carousel' && instance.provider === 'whaileys') {
        const cards = Array.isArray(content) ? content : (content.cards || []);
        sentMessage = await req.waManager.sendCarousel(instance.id, remoteJid, cards);
      } else if (messageType === 'buttons' && instance.provider === 'whaileys') {
        sentMessage = await req.waManager.sendButtons(instance.id, remoteJid, content.text, content.footer, content.buttons);
      } else if (messageType === 'list' && instance.provider === 'whaileys') {
        sentMessage = await req.waManager.sendListMessage(instance.id, remoteJid, content.text, content.buttonText, content.sections, content.footer);
      } else if (messageType === 'audio' && typeof req.waManager.sendAudio === 'function') {
        sentMessage = await req.waManager.sendAudio(instance.id, remoteJid, waMessage.audio.url, { ptt: true, mimetype: waMessage.mimetype });
      } else {
        // Caso genérico
        const result = await req.waManager.sendMessage(instance.id, remoteJid, waMessage);
        sentMessage = result;
      }
    } catch (err) {
      console.error("Erro interno WA Manager:", err);
      throw err;
    }

    // Insert message into DB
    await db.run(`INSERT INTO messages (id, ticket_id, contact_id, instance_id, type, content, wa_message_id, wa_remote_jid, from_me, timestamp, status, media_url, media_filename) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [messageId, ticket.id, contact.id, instance.id, messageType, messageContent, sentMessage.key?.id || messageId, sentMessage.key?.remoteJid || remoteJid, true, timestamp.toISOString(), 'sent', mediaUrl, mediaFilename]);

    res.status(201).json({ messageId, status: 'sent', instanceUsed: instance.name });

  } catch (error) {
    console.error('❌ Erro send-quick:', error);
