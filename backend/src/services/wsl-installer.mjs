...
export class WSLInstaller {
  ...
  async cleanPreviousInstallations() {
    ...
    try {
      ...
      try {
        await execAsync('powershell -Command "Remove-Item -Path \\"$env:LOCALAPPDATA\\Packages\\CanonicalGroupLimited*\\" -Recurse -Force -ErrorAction SilentlyContinue"');
      } catch {}
  ...
  async install() {
    try {
      ...
      if (!await this.isAdmin()) {
        throw new Error('Permissões de administrador necessárias. Por favor, execute como administrador.');
      }
